...
window.onload = async () => {
    ...
    if (id) {
        ...
        selectedId = id;
        try {
            const response = await axios.get(`${BASE_URL}/instructors/${id}`);
            ...
            firstnameDOM.value = instructor.firstname;
            lastnameDOM.value = instructor.lastname;
            emailDOM.value = instructor.email;
        } catch (error) {
...
const submitData = async () => {
    ...
    try {
        ...
        const errors = validateData(instructorData);
        if (errors.length > 0) {
            throw {
        ...
        if (mode === 'create') {
            await axios.post(`${BASE_URL}/instructors`, instructorData);
        } else if (mode === 'edit') {
            await axios.put(`${BASE_URL}/instructors/${selectedId}`, instructorData);
